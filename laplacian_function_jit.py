...
def sumit(inp: List[torch.Tensor]):
  base = inp[0]
  for i in range(1, len(inp)):
    base = base + inp[i]
  return base
    
def laplacian_from_log_domain(xs, network):
  """ 
  ...
  lap_ys_components = [torch.autograd.grad(dy_dxi, xi, ones, retain_graph=True, create_graph=False)[0] \
...
laplacian_from_log_domain=torch.jit.script(laplacian_from_log_domain)
